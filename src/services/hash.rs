...
pub fn verify_client_hash(file_bytes: &[u8], client_hash_hex: &str) -> Result<[u8; 32], AppError> {
    let server_hash = compute_sha256(file_bytes);
    let client_hash = hex::decode(client_hash_hex).map_err(|_| AppError::InvalidHash)?;
