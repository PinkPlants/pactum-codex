...
pub async fn run(state: AppState) {
    ...
    loop {
        match connect_and_listen(ws_url, program_id, &state).await {
            Ok(()) => {
...
async fn handle_sign_agreement(
    ...
    // Notify all parties
    let parties: Vec<String> = sqlx::query_scalar(
        "SELECT party_pubkey FROM agreement_parties WHERE agreement_pda = $1",
    )
    .bind(agreement_pda)
    .fetch_all(&state.db)
    .await?;
...
async fn handle_cancellation(
    ...
    // Update status
    sqlx::query(
        "UPDATE agreement_parties SET status = $1 WHERE agreement_pda = $2"
    )
    .bind(status)
    .bind(agreement_pda)
    .execute(&state.db)
